...
public class UserSceneController implements Initializable{
    ...
    private void loadShoppingList(){
        ...
        // SELECT p.name, COUNT(up.quantity), SUM(p.price) 
        var query = ctx.createQuery(Product.class,
        "FROM Product p LEFT JOIN UserProduct up ON up.id_product = p.id_product  WHERE up.id_user = :id_user GROUP BY p.name")
            .setParameter("id_user", id_user);
        
        List<Product> list = query.getResultList();
        
        tbTable.setItems(FXCollections.observableArrayList(list));
    }
